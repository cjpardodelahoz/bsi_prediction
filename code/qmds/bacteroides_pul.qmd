...
# Filter to patients who received quinolone prophylaxis and have post samples
features_post <- patient_features %>%
  filter(received_quin, n_samples_post > 0)

# Stratified train/test split (70/30) by infection status
set.seed(87433)
split_by_case <- function(df, case_col = "has_infection", prop = 0.7) {
  cases <- df %>% filter(.data[[case_col]] == TRUE)
  controls <- df %>% filter(.data[[case_col]] == FALSE)
  n_cases <- floor(nrow(cases) * prop)
  n_controls <- floor(nrow(controls) * prop)
  train_cases <- sample(seq_len(nrow(cases)), n_cases)
  train_controls <- sample(seq_len(nrow(controls)), n_controls)
  train <- bind_rows(cases[train_cases, ], controls[train_controls, ])
  test <- bind_rows(cases[-train_cases, ], controls[-train_controls, ])
  list(train = train, test = test)
}
splits <- split_by_case(features_post)

# Model formulas
model_formulas <- list(
  model1 = has_infection ~ proteobacteria_detected_post,
  model2 = has_infection ~ log10(median_proteobacteria_post + 1e-05),
  model3 = has_infection ~ ASV_277_abund_detected_post,
  model4 = has_infection ~ log10(median_bacteroidaceae_post + 1e-05),
  model5 = has_infection ~ log10(median_proteobacteria_post + 1e-05) + log10(median_bacteroidaceae_post + 1e-05)
)

# Fit models on training set
models <- lapply(model_formulas, function(form) {
  glm(form, data = splits$train, family = binomial)
})
...
evaluate_model <- function(model, train, test) {
  ...
  # ROC and AUC
  train_roc <- pROC::roc(train$has_infection, train_pred)
  test_roc <- pROC::roc(test$has_infection, test_pred)
  tibble(
...
results <- map2(models, names(models), function(model, name) {
  eval <- evaluate_model(model, splits$train, splits$test)
  tibble(model = name, train_auc = eval$train_auc, test_auc = eval$test_auc)
}) %>% bind_rows()

print(results)

# Plot ROC curves for each model (test set)
par(mfrow = c(2, 3))
for (i in seq_along(models)) {
  test_pred <- predict(models[[i]], newdata = splits$test, type = "response")
  roc_obj <- pROC::roc(splits$test$has_infection, test_pred)
  plot(roc_obj, main = names(models)[i])
}

# Save results
write_csv(results, "document/tables/model_comparison_auc.csv")
```
...
# --- 1. Prepare patient-level data and split into train/test as before ---

# Only patients with quinolone and post samples
features_post <- patient_features %>%
  filter(received_quin, n_samples_post > 0)

# Stratified train/test split (by patient, 70/30)
set.seed(87433)
split_by_case <- function(df, case_col = "has_infection", prop = 0.75) {
  cases <- df %>% filter(.data[[case_col]] == TRUE)
  controls <- df %>% filter(.data[[case_col]] == FALSE)
  n_cases <- floor(nrow(cases) * prop)
  n_controls <- floor(nrow(controls) * prop)
...
splits <- split_by_case(features_post)
test_patient_ids <- splits$test$PatientID

# --- 2. For each cutoff, recalculate post-prophylaxis features for test patients ---

# Define cutoffs (relative to HCT)
cutoff_days <- -2:5

# Use microb_features and patientday tables from your script
# microb_features: sample-level features (already joined with taxa abundances)
# quin_start, first_infection, engraftment_day: already defined in your script

# Function to recalculate features for a given cutoff
recalc_patient_features_cutoff <- function(cutoff, test_patient_ids) {
  # Subset to test patients and relevant samples
  microb_features %>%
    ...
    mutate(
      has_infection = !is.na(first_infection_day),
      received_quin = !is.na(first_quin_day),
      # For each patient, the upper bound for samples is the minimum of:
      # - cutoff day (relative to HCT)
      # - first infection day (if infected)
      # - engraftment day (if not infected)
      max_day = pmin(
        cutoff,
    ...
    filter(
      received_quin,
      DayRelativeToNearestHCT >= (first_quin_day + 1),
      DayRelativeToNearestHCT < max_day
    ) %>%
    group_by(PatientID, has_infection, received_quin) %>%
    summarise(
      median_proteobacteria_post = median(Proteobacteria_abund, na.rm = TRUE),
      proteobacteria_detected_post = any(Proteobacteria_abund >= 0.01, na.rm = TRUE),
      median_ASV_277_abund_post = median(ASV_277_abund, na.rm = TRUE),
      ASV_277_abund_detected_post = any(ASV_277_abund >= 0.01, na.rm = TRUE),
    ...
    # Add infection status for test patients
    left_join(
      splits$test %>% select(PatientID, has_infection),
      by = "PatientID",
      suffix = c("", ".label")
    ) %>%
    mutate(has_infection = coalesce(has_infection.label, has_infection)) %>%
    select(-has_infection.label)
}

# Generate a list of test sets for each cutoff
test_sets_by_cutoff <- setNames(
  lapply(cutoff_days, recalc_patient_features_cutoff, test_patient_ids = test_patient_ids),
  paste0("cutoff_", cutoff_days)
)

# --- 3. Train models on the training set (unchanged) ---

# Use the same formulas as before
model_formulas <- list(
  model1 = has_infection ~ proteobacteria_detected_post,
  model2 = has_infection ~ log10(median_proteobacteria_post + 1e-05),
  model3 = has_infection ~ ASV_277_abund_detected_post,
...
# --- 4. Evaluate models on each test set (for each cutoff) ---

evaluate_model_cutoffs <- function(model, test_sets) {
  map_dfr(names(test_sets), function(name) {
    test_set <- test_sets[[name]]
    # Only evaluate if there are no missing predictors and at least 2 outcome classes
    if (nrow(test_set) > 0 &&
        length(unique(test_set$has_infection)) > 1 &&
        all(all.vars(formula(model)) %in% names(test_set))) {
      test_pred <- predict(model, newdata = test_set, type = "response")
      roc_obj <- tryCatch(
        pROC::roc(test_set$has_infection, test_pred),
        error = function(e) NULL
      )
...
# Evaluate all models across all cutoffs
all_auc_results <- map2(models, names(models), function(model, model_name) {
  aucs <- evaluate_model_cutoffs(model, test_sets_by_cutoff)
  aucs$model <- model_name
  aucs
}) %>% bind_rows()
...
auc_wide <- all_auc_results %>%
  mutate(cutoff_num = as.numeric(str_remove(cutoff, "cutoff_"))) %>%
  select(model, cutoff_num, auc, n)

# Plot AUC vs cutoff for each model
library(ggplot2)
ggplot(auc_wide, aes(x = cutoff_num, y = auc, color = model)) +
  geom_line() +
  geom_point() +
  labs(
    x = "DayRelativeToNearestHCT cutoff (max day included)",
    y = "Test set AUC",
    color = "Model",
    title = "Model AUC vs. cutoff day for test set feature calculation"
  ) +
  theme_minimal()

# Save results
write_csv(auc_wide, "document/tables/model_auc_by_cutoff.csv")
```
...
# --- 1. Prepare patient-level data as before ---
features_post <- patient_features %>%
  filter(received_quin, n_samples_post > 0)

# --- 2. Create 5 stratified folds by patient infection status ---
set.seed(87433)
n_folds <- 5

# Assign folds stratified by has_infection
features_post <- features_post %>%
  group_by(has_infection) %>%
  mutate(fold = sample(rep(1:n_folds, length.out = n()))) %>%
  ungroup()

# --- 3. Define cutoffs and model formulas ---
cutoff_days <- -3:5
...
for (cutoff in cutoff_days) {
  ...
  for (fold_n in 1:n_folds) {
    ...
    # Recalculate features for test patients at this cutoff
    test_set <- recalc_patient_features_cutoff(cutoff, test_patient_ids)
    ...
    for (model_name in names(model_formulas)) {
      ...
      # Only fit if all predictors are present in train and test
  ...
  # Aggregate AUCs for this cutoff
  for (model_name in names(model_formulas)) {
...
ggplot(cv_auc_results_tbl, aes(x = cutoff, y = mean_auc, color = model)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = mean_auc - se_auc, ymax = mean_auc + se_auc), width = 0.2) +
  labs(
    x = "DayRelativeToNearestHCT cutoff (max day included)",
    y = "Mean test set AUC (5-fold CV)",
    color = "Model",
    title = "Cross-validated Model AUC vs. cutoff day"
  ) +
  theme_minimal()

write_csv(cv_auc_results_tbl, "document/tables/model_auc_by_cutoff_cv.csv")
```

```{r}
# Define custom model colors
model_colors <- c(
  "model1" = "#e15454",
  "model2" = "#4ae1d9",
  "model3" = "#2c5362",
...
ggsave("document/plots/linearmodels_auc.pdf", linearmodels_auc, width = 7, height = 5)
```
