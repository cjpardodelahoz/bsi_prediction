...
# Table with ASVs and infection data
...
asv_genera_bacteroides_species <- asv_genera_bacteroides_species %>%
  filter(!identity %in% gutborne_pathogens)

# Prepare output columns
asv_genera_bacteroides_species$glm_coef <- NA
asv_genera_bacteroides_species$glm_pval <- NA
asv_genera_bacteroides_species$glmer_coef <- NA
asv_genera_bacteroides_species$glmer_pval <- NA

# Loop through each ASV
for (i in seq_len(nrow(asv_genera_bacteroides_species[1:2,]))) {
  ...
  if (asv_col %in% colnames(tblrel_asv_meta)) {
    ...
    # GLM
    glm_fit <- glm(Escherichia_infection ~ ASV_abund + factor(DayRelativeToNearestHCT),
                   data = df, family = binomial)
    glm_coef <- coef(summary(glm_fit))["ASV_abund", "Estimate"]
    glm_pval <- coef(summary(glm_fit))["ASV_abund", "Pr(>|z|)"]
    
    # GLMER
    glmer_fit <- glmer(Escherichia_infection ~ ASV_abund + factor(DayRelativeToNearestHCT) + (1 | PatientID),
                       data = df, family = binomial)
    glmer_coef <- coef(summary(glmer_fit))["ASV_abund", "Estimate"]
    glmer_pval <- coef(summary(glmer_fit))["ASV_abund", "Pr(>|z|)"]
    
    # Store results
    asv_genera_bacteroides_species$glm_coef[i] <- glm_coef
    asv_genera_bacteroides_species$glm_pval[i] <- glm_pval
