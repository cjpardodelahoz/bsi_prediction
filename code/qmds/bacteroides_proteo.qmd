...
### BSI pathogen frequency

For one of the figures, I prepared a stacked bar plot of the frequency of BSIs caused by different pathogens. I considered only the timeframe between -10 and 50 days relative to the HCT:
...
gutborne_infections <- filtered_infections %>%
  rowwise() %>%
  mutate(
    DayRelativeToNearestHCT = calculate_day_relative_to_nearest_hct(PatientID, Day, tblhctmeta),
    genus = case_when(
    InfectiousAgent %in% enterococcus ~ "Enterococcus",
    InfectiousAgent %in% klebsiella ~ "Klebsiella",
    TRUE ~ InfectiousAgent
    )
  ) %>%
  ungroup() %>%
  filter(between(DayRelativeToNearestHCT, -10, 50) & InfectiousAgent %in% gutborne)

  # Tally number of unique patients with infection per genus
  genus_patient_tally <- gutborne_infections %>%
    distinct(PatientID, genus) %>%
    count(genus, name = "n_patients") %>%
    arrange(n_patients) # ascending order for stacking (lowest at top, highest at bottom)

  # Set factor levels so highest frequency is at the bottom of the stack
  genus_patient_tally$genus <- factor(genus_patient_tally$genus, levels = genus_patient_tally$genus)

  genus_patient_tally
  # Define fill colors: Enterococcus green, Escherichia red, others as different shades of red
  genus_colors <- c(
    "Enterococcus" = "#1c8c3a",
    "Escherichia" = "#fc4242",
    "Klebsiella" = "#fc6060",
    ...
    "Citrobacter" = "#FBBDBD",
    "Pseudomonas" = "#f77c7c",
    "Stenotrophomonas_Maltophilia" = "#faa2a2"
  )

# Convert patietn counts to frequency
genus_patient_tally <- genus_patient_tally %>%
  mutate(freq = n_patients / sum(n_patients))

# Stacked bar plot (one bar, stacked by genus, y axis as frequency)
ggplot(genus_patient_tally, aes(x = 1, y = freq, fill = genus)) +
  geom_bar(stat = "identity", width = 0.6) +
  scale_fill_manual(values = genus_colors, na.value = "#FDEAEA") +
  labs(
    x = NULL,
    y = "BSI frequency",
    fill = "Genus"
...
# Save the plot
ggsave("document/plots/patient_tally_by_genus.pdf", width = 3.8, height = 4)
