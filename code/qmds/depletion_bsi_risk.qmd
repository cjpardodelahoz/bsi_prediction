...
This is the proportion of stool samples with >10% *Streptococcus* relative abundance over time relatibe to the transplant:
...
streptococcus_domination <- tblrel_genus_meta %>%
  filter(Streptococcus_abund > 0.15) %>%  # Filter for domination
  group_by(PatientID) %>%
  summarise(
    DominationDay = ifelse(
      any(DayRelativeToNearestHCT >= -1 & DayRelativeToNearestHCT <= 1),
      min(DayRelativeToNearestHCT[DayRelativeToNearestHCT >= -10 & DayRelativeToNearestHCT <= 10], na.rm = TRUE),
...
observation_window <- seq(-10, 50)

# Expand the data to include one row per day for each patient
expanded_data <- tblpatientday_clinical %>%
  mutate(DayRelativeToNearestHCT = as.numeric(DayRelativeToNearestHCT)) %>%  # Ensure numeric
  complete(PatientID, DayRelativeToNearestHCT = observation_window) %>%  # Fill in missing days
...
cox_data_time_dependent <- expanded_data %>%
  ...
  group_by(PatientID) %>%
  mutate(
    start_time = lag(DayRelativeToNearestHCT, default = min(observation_window)),  # Start of the interval
