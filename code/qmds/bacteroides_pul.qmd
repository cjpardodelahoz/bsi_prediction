...
```{r}
# Prevalence and abundance after quin start
post_tbl <- tbl_prevalence %>%
...
I trained classifiers to predict whether a patient had an infection at any point between the start of quinolones and engraftment using the patient's median post-quinolone abundance of different taxa (Pseudomonadota, Bacteroidaceae, Tannerellaceae) or their interaction terms. Then, I tested model performance with medians calculated from increasingly larger sets of samples from the test patients, using the day relative to transplant as cutoffs. For this model evaluation, I needed a fucntion that would recalculate the patient medians for each given test cutoff:
...
Then, I calcualted the ROC-AUCs from 5-fold cross-validation (i.e., splitting data in 5 folds stratified by infeciton status to preserve label proportions; train on 4, test on remaining 1; repeat across all 5 folds)

```{r}
# Library to extract ROC-AUCs
library(pROC)

# Prepare patient-level data as before
features_post <- patient_features %>%
  filter(received_quin, n_samples_post > 0)

# Create 5 stratified folds by patient infection status
set.seed(603)
n_folds <- 5
...
# Define cutoffs and model formulas
cutoff_days <- -3:5
...
# Aggregate results in a single table
cv_auc_results_tbl <- bind_rows(cv_auc_results)
```

Finally, I plotted the ROC-AUCs from the different models across the different time cutoffs:
...
# Plot ROC-AUCs across thresholds
