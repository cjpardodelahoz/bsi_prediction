...
```{bash, eval=FALSE}
sbatch code/scripts/bacteroides_variation/yan_sd_2022_vamb_multi.sh #6460458
sbatch code/scripts/bacteroides_variation/bacteroides_vamb_multi.sh #6451791
```
...
sbatch code/scripts/bacteroides_variation/yan_sd_2022_gtdb.sh
sbatch code/scripts/bacteroides_variation/bacteroides_gtdb.sh
```
...
targeted_gtdb <- read_tsv("analyses/bacteroides_variation/binning/gtdb/gtdbtk.bac120.summary.tsv")
meta_gtdb <- read_tsv("analyses/yan_sd_2022/binning/gtdb/gtdbtk.bac120.summary.tsv")
full_gtdb <- bind_rows(targeted_gtdb, meta_gtdb)

# Load CheckM2 results
checkm <- read_tsv("analyses/bacteroides_variation/genomes/checkm2/quality_report.tsv")
...
bacteroides_mag_data <- checkm %>%
  ...
  filter(Completeness >= 90 & Contamination <= 5 & str_detect(classification, "g__Bacteroides"))
...
```{bash, eval=FALSE}
sbatch code/scripts/bacteroides_variation/bacteroides_dbcan.sh #6276228
```