...
# Plot (log odds ratio)
bacteroidota_detection <- ggplot(prevalence_effects, aes(x = factor(family, levels = c("Bacteroidaceae", "Tannerellaceae", "Prevotellaceae", "Rikenellaceae", "Barnesiellaceae", "Marinifilaceae", "Dysgonomonadaceae")), y = estimate, ymin = conf.low, ymax = conf.high)) +
...
# Align iqr plots to the left of the tree
cgc_iqr_plot <- bacteroidaceae_cgc_iqr %>% insert_left(bacteroidaceae_tree_plot, width = 0.75)
gh_iqr_plot <- gh_iqr %>% insert_left(family_tree_plot, width = 0.75)
pl_iqr_plot <- pl_iqr %>% insert_left(family_tree_plot, width = 0.75)
signalp_cgc_iqr_plot <- signalp_cgc_iqr %>% insert_left(family_tree_plot, width = 0.75)
signalp_gh_iqr_plot <- signalp_gh_iqr %>% insert_left(family_tree_plot, width = 0.75)
