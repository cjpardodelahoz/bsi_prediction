...
This is an initial exploration of the dynamics of *Enteroccocus* diversity in the HCT patient population. I have the following goals:
 ...
 ## Summary of dominations and infections
...
```{bash, eval=FALSE}
# Merge fastas from dereplicated genomes
...
# Shorten fasta headers for Prokka
sed -i "s|_cov.*||" analyses/enterococcus_diversity/strains/ref_contigs/ref_contigs.fna

# Extract contig names from the dereplicated E. faecium MAG fasta and save to a list
grep '^>' analyses/enterococcus_diversity/genomes/drep/dereplicated_genomes/s_1044K_1_meta_vamb_single.fna | \
  sed 's/^>//' > analyses/enterococcus_diversity/strains/ref_contigs/ref_contig_list.txt
#grep '^>' analyses/enterococcus_diversity/genomes/drep/dereplicated_genomes/s_1044K_1_meta_vamb_single.fna | \
#  sed 's/^>//' | \
#  cut -d '_' -f '1 2 3 4' > analyses/enterococcus_diversity/strains/ref_contigs/ref_contig_list.txt
```

Run Prokka aanotation on the set of E. faecium reference contigs using the labeling from [*E. faecium* strain DO](https://www.ncbi.nlm.nih.gov/nuccore/CP003583):
...
# Annotate E. faecium reference contigs
sbatch code/scripts/enterococcus_diversity/prokka_ref_contigs.sh

# Annotate all E. faecium MAGs with Prokka
sbatch code/scripts/enterococcus_diversity/prokka_efaecium_nc_mags.sh
```

I ran both the inStrain pipeline for popANI and microdiversity calculations using the *E. faecium* genome `s_1044K_1_meta_vamb_single` as a target and the dereplicated genomes as a reference library for competitive mapping:
...
I extracted the translated sequences of *pbp5* and *psr* from the *E. faecium* MAGs from patient 1179:

```{bash, eval=FALSE}
# Directory for sequences
OUT_SEQ_DIR="analyses/enterococcus_diversity/beta_lactam/s_1179/seqs"
mkdir -p ${OUT_SEQ_DIR}

# List all .gbk files for patient 1179
GBK_1179=$(find analyses/enterococcus_diversity/genomes/prokka -type f -name "s_1179*.gbk")

# Biopython required for my python script
conda activate biopython

# Extract the sequences
python software/custom/extract_gene_aa.py ${GBK_1179} \
  -g pbp5 psr \
  -o ${OUT_SEQ_DIR} \
  -p 1
```
 
For patient 1179, all genomes had identical PBP5 and PSR sequences with all almost all critical AAs matching the PBP5-R consensus (Pietta et al., 2014), including i466S and A485. The exception was residue V586, which matched PBP5-S.

## Patient sample selection for additional experiments
...
### Sample selection for long-read metagenomes and strain isolation (pilot)

Based on the results above, I will target three patients:

  - 1179
  - 1044
  - 668

The goal was to obtain four stool samples per patient, i.e., two on each side of the strain shift timepoint. I prepared a table with MRNs and collection dates for Jen/Zoe to check availability:

```{r}
# Selected patients
pilot_patients <- c("1179", "1044", "668")

# Table with sample info
pilot_samples <- meta_tblASVsamples_with_dates %>%
  filter(PatientID %in% pilot_patients) %>%
  mutate(SampleID = str_remove(SampleID, "s_")) %>%
  mutate(SampleGroup = case_when(
                                  PatientID == "1179" & between(DayRelativeToNearestHCT, 3, 10) ~ "group1",
                                  PatientID == "1179" & between(DayRelativeToNearestHCT, 11, 40) ~ "group2",
                                  PatientID == "1044" & between(DayRelativeToNearestHCT, -3, 5) ~ "group3",
                                  PatientID == "1044" & between(DayRelativeToNearestHCT, 6, 18) ~ "group4",
                                  PatientID == "668" & between(DayRelativeToNearestHCT, 7, 40) ~ "group5",
                                  PatientID == "668" & between(DayRelativeToNearestHCT, 41, 60) ~ "group6",
                                  TRUE ~ "notincluded"
  )) %>%
  select(MRN, collectionDate, PatientID, SampleID, SampleGroup) %>%
  filter(SampleGroup != "notincluded") %>%
  arrange(SampleGroup)

# Save table
write_csv(pilot_samples, "document/tables/pilot_sample_info.csv")
```
...
isabl get-metadata analyses \
  ...
  -fi targets.sample.identifier.in 668Z,668X \
  --field pk \
