...
microb_features <- tblrel_phylum_meta %>%
...
fit_taxon_models <- function(taxon_name, data) {
  ...
  tryCatch({
    ...
    if (nrow(features_post) > 0) {
      ...
      results$median_post <- tidy(model) %>%
  ...
  tryCatch({
    ...
    if (nrow(features_post) > 0) {
      ...
      results$detection_post <- tidy(model) %>%
        filter(term != "(Intercept)") %>%
        mutate(
  ...
  tryCatch({
    ...
    if (nrow(features_prepost) > 0) {
      ...
      results$median_prepost <- tidy(model) %>%
        filter(term != "(Intercept)") %>%
        mutate(
  ...
  tryCatch({
    ...
    if (nrow(features_prepost) > 0) {
      ...
      results$detection_prepost <- tidy(model) %>%
        filter(term != "(Intercept)") %>%
        mutate(
  ...
  tryCatch({
    ...
    if (nrow(features_all) > 0) {
      ...
      results$median_full <- tidy(model) %>%
        filter(term != "(Intercept)") %>%
        mutate(
  ...
  tryCatch({
    ...
    if (nrow(features_all) > 0) {
      ...
      results$detection_full <- tidy(model) %>%
        filter(term != "(Intercept)") %>%
        mutate(
